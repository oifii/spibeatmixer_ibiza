...
int main(int argc, char *argv[])
{
	...
	*/
	...
	/*
	//////////////////////////////////////////////////////////////////////////////////////////
	//for each instrument, pick random wavset, loop wavset for loopduration_s seconds and play
	//////////////////////////////////////////////////////////////////////////////////////////
	Instrument* pAnInstrument = NULL;
	WavSet* pAWavSet = NULL;
	for(int i=0; i<100; i++) //repeat 10 times
	{
			...
			for(it=pInstrumentSet->instrumentvector.begin();it<pInstrumentSet->instrumentvector.end();it++)
			{
				...
				if(pAnInstrument2)
				{
					...
					if(pAWavSet2)
					{
						...
						pSilentWavSet->SpreadSample("10110100", pAWavSet2, loopduration_s, 5, 0.0); //pattern over loopduration_s sec, sample duration 5sec and with pattern offset of 0sec;// pAWavSet2
					}
				}
				pSilentWavSet->Play(&outputParameters);
	...
	for(int i=0; i<100; i++) //repeat 100 times
	{
		if(pInstrumentSet && pInstrumentSet->HasOneInstrument()) 
		{
			...
			if(pAnInstrument2)
			{
				...
				//{
					//pSilentWavSet->SpreadSample("1111110011101100", pAWavSet2, loopduration_s, 0.5, 0.0); //pattern over loopduration_s sec, sample duration 0.5sec and with pattern offset of 0sec;// pAWavSet2
					pSilentWavSet->SpreadSamples("10110100", pAnInstrument2, loopduration_s, 5, 0.0); //pattern over loopduration_s sec, sample duration 5sec and with pattern offset of 0sec;// pAWavSet2
				//}
			}
